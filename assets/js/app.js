...
function calculatorInit() {
    ...
    // digits event
    digitsEl.on(eventType, function () {
        // reset steps 
        stepsEl.text('');
    ...
    // AC - reset btn
    clearBtn.on(eventType, function () {
        reset();
    });
    ...
    saveBtn.on(eventType, function () {
        if (saveData !== '') {
            var jqxhr = $.post("/api/save", { total: saveData + total }, function (data) {
                reset();
                saveData = '';
                total = 0;
                alert("Your calculation has been saved successfully!");
            }).fail(function (err) {
                console.log(err)
            });
        }
    ...
    // operator btn event
    operatorBtn.on(eventType, function () {
    ...
    function getTotal(array) {
        ...
        currentValue = [total];
        currentValueEl.text(total.toString().split('.').join(','))
    }
